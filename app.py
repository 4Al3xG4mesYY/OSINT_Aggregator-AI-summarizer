...
@app.route('/archive')
def archive():
    """New route to display the full archive of all articles."""
    conn = get_db_connection()
    # Query all articles, ordering by the most recently added first
    all_articles = conn.execute('SELECT * FROM articles ORDER BY created_at DESC').fetchall()
    conn.close()
    return render_template('archive.html', articles=all_articles)
